...
func BuildUsageFacts(output map[string]any) []map[string]any {
	provider := stringValue(output["llm_provider"])
	...
	return []map[string]any{
		{
			"resource_type":  "llm",
			"provider":       provider,
			"model":          firstNonEmptyString(stringValue(output["llm_model"]), stringValue(output["model"])),
			...
			"usage_breakdown": map[string]any{
				...
				"fallback_hops":     output["llm_fallback_hops"],
			},
		},
	}
}
